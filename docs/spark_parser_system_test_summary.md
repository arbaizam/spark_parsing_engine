...
| ST-003 | Domain display profiles | Bounded `title_business_v1` exceptions and numeric-hyphen casing, approved interest-rate canonicalization, action-free string changes in audit, and explicit preservation of an unknown interest-rate value materialize correctly. |
...
| ST-006 | DataFrame output contract | Scalar target order, result keys, prefixed result columns, parser identity, content hash, engine version, and the fixed scalar audit fields match the public contract. |
