...
| ST-003 | Domain display profiles | Bounded `title_business_v1` rules, approved interest-rate canonicalization, and `property_type_v1` aliases materialize correctly. Property values are checked against each fixture's stable row ID, including corrected `Condominium`, `Four-Unit`, retained `LIHTC`, mixed-use restructuring with retained descriptors, action-free canonical changes, and explicit preservation of unknown non-mixed values. |
