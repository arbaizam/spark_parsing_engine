...
| ST-004 | Error policies and lazy execution | Null, default, preserve, and nested child-error policies produce their configured values and actions, while `fail` raises a supported Spark exception containing the exact source, target, and datatype context only when the parsed value is materialized. |
